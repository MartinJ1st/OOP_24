...
private:
    ...
    float price;
public:
    Busline(char *departureCity = "Skopje", char *arrivalCity = "Negotino", int departureTime = 1800,
            int arrivalTime = 1930, float price = 7) {
    ...
    bool operator==(Busline &other) {
        return strcmp(this->departureCity, other.departureCity) == 0 &&
               strcmp(this->arrivalCity, other.arrivalCity) == 0;
    }
};
...
public:
    ...
    friend void searchBuslines(BusOperator b, char departureCity[], char arrivalCity[]);
};

void searchBuslines(BusOperator b, char departureCity[], char arrivalCity[]) {
    Busline tmp(departureCity, arrivalCity, 0, 0, 0);
    //dali letovi[i]==tmp
    for (int i = 0; i < b.count; ++i) {
        if (b.buslines[i] == tmp) {
            b.buslines[i].printBusline();
            return;
        }
    }

    for (int i = 0; i < b.count; ++i) {
        if (strcmp(b.buslines[i].getDepartureCity(), departureCity) == 0) {
            for (int j = 0; j < b.count; ++j) {
                if (strcmp(b.buslines[j].getDepartureCity(), b.buslines[i].getArrivalCity()) == 0 &&
                    strcmp(b.buslines[j].getArrivalCity(), arrivalCity) == 0) {
                    cout << "-------------" << endl;
                    b.buslines[i].printBusline();
                    b.buslines[j].printBusline();
                }
            }
        }
    }
