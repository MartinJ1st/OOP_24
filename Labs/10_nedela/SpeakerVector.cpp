...
private:
    ...
    int duration;
public:
    Speaker(const string &name = "", const string &title = "", int duration = 0) : name(name), title(title),
                                                                                   duration(duration) {}
...
private:
    ...
    void copy(const Conference &other) {
...
public:
    ...
    int getConferenceDuration() {
        int sum = 0;
        for (const Speaker &speaker: speakers) {
            sum += speaker.getDuration();
        }
        return sum;
    }

    Conference &operator+=(const Speaker &s) {
        if (getConferenceDuration() + s.getDuration() > MAX_DURATION) {
    ...
    void longestPresentation() {
        Speaker longest = speakers[0];
        for (const Speaker &speaker: speakers) {
            if (speaker.getDuration() > longest.getDuration()) {
                longest = speaker;
            }
        }
    ...
    friend ostream &operator<<(ostream &out, const Conference &c) {
        out << c.code << " - " << c.date << ", " << c.speakers.size();
        return out;
    }
    ...
    const vector<Speaker> &getSpeakers() const {
        return speakers;
    }

    void setSpeakers(const vector<Speaker> &speakers) {
    ...
    static void setMaxDuration(int maxDuration) {
        Conference::MAX_DURATION = maxDuration;
    }
};
